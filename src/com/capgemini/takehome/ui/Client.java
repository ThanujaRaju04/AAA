...
public class Client {
	...
	public static void main(String[] args) throws BSAException {
		...
		case 1:
			...
			boolean flag = service.isValidproductCode(productCode);

			if (flag && ((productCode <= 1004) && (productCode >= 1001))) {
				System.out.println("\nEnter the quantity: ");
				int quantity = 0;
				try {
				...
				if (quantity >= 1) {
					List<ProductBean> list = service.getProductDetails(productCode);
				...
				System.out
						.println("The ProductCode is not available in DataBase\n");
			}
